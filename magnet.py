...
class MagNet3Frames(object):
    ...
    def run(self,
        ...
        vid_frames = sorted(glob(r"E:\share\dataset\lipreading\LRW\graph\data_frames_output\*\*\*\*.png"))
        print(vid_frames)
        ...
        for frame in tqdm(vid_frames):
            ...
            out_amp = self.inference(frame, frame, amplification_factor)

            # im_path = os.path.join(out_dir, file_name)
            save_images(out_amp, [1, 1], frame.replace("\\data_frames_raw\\", "\\data_frames_output\\"))
            # if velocity_mag:
            #     prev_frame = frame
